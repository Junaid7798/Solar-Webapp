...
import { Calculator as CalcIcon, Zap, Home, Building2, Leaf } from 'lucide-react';

const UNITS_PER_KW_PER_MONTH = 115;
const SQFT_PER_KW = 90;
const COST_PER_KW = 60_000;
const CO2_KG_PER_UNIT = 0.82;
const TREES_PER_TONNE_CO2 = 45;
const MIN_SYSTEM_KW = 1;
const COMMERCIAL_RATE = 11.5;
const MIN_ROOF_FOR_MIN_SYSTEM = SQFT_PER_KW * MIN_SYSTEM_KW;
...
const EMPTY_RESULT: EstimateResult = {
  systemSize: 0,
  monthlySavings: 0,
  panels: 0,
  roofNeeded: 0,
...
export const Calculator = () => {
  ...
  const [results, setResults] = useState<EstimateResult>(EMPTY_RESULT);

  const roofTooSmall = roof < MIN_ROOF_FOR_MIN_SYSTEM;
  const maxRoofSupportedKw = useMemo(() => Math.round((roof / SQFT_PER_KW) * 10) / 10, [roof]);

  const calculateEstimate = () => {
    const units = type === 'residential' ? deriveResidentialUnitsFromBill(bill) : bill / COMMERCIAL_RATE;

    if (roofTooSmall) {
      setResults({
        ...EMPTY_RESULT,
        systemSize: maxRoofSupportedKw,
        panels: Math.max(1, Math.ceil(maxRoofSupportedKw * 2)),
        roofNeeded: MIN_ROOF_FOR_MIN_SYSTEM,
        message: `This roof supports about ${maxRoofSupportedKw} kW, but at least ${MIN_SYSTEM_KW} kW (${MIN_ROOF_FOR_MIN_SYSTEM} sq.ft) is recommended for a practical system.`,
      });
      setIsCalculated(true);
      return;
    }

    let systemSize = units / UNITS_PER_KW_PER_MONTH;
    systemSize = Math.min(systemSize, roof / SQFT_PER_KW);

    if (phase === 'single') {
      systemSize = Math.min(systemSize, 5);
    }

    systemSize = Math.max(MIN_SYSTEM_KW, Math.round(systemSize * 10) / 10);

    const baseCost = systemSize * COST_PER_KW;
    let subsidy = 0;

    if (type === 'residential') {
      if (systemSize <= 2) subsidy = systemSize * 30_000;
      else if (systemSize <= 3) subsidy = 60_000 + (systemSize - 2) * 18_000;
      else subsidy = 78_000;
    }
    ...
    const annualUnits = systemSize * UNITS_PER_KW_PER_MONTH * 12;
    const co2 = (annualUnits * CO2_KG_PER_UNIT) / 1000;
    const trees = co2 * TREES_PER_TONNE_CO2;

    setResults({
      systemSize,
      monthlySavings: Math.round(monthlySavings),
      panels: Math.ceil(systemSize * 2),
      roofNeeded: Math.ceil(systemSize * SQFT_PER_KW),
      ...
      feasible: true,
      message: type === 'commercial' ? 'Commercial systems usually need a site-specific tariff and load audit before final pricing.' : 'Subsidy values are indicative and depend on the latest central and state policy eligibility.',
    });
    setIsCalculated(true);
  };
    ...
    <section id="calculator" className="py-24 bg-sky-deep relative overflow-hidden">
      ...
      <div className="max-w-7xl mx-auto px-6 md:px-12 relative z-10">
        <div className="text-center mb-16">
          ...
          <h2 className="font-display font-bold text-4xl md:text-5xl text-white uppercase tracking-wide">{t('calculator', 'title')}</h2>
        </div>
            ...
            <div className="mb-8 bg-sky-deep/50 p-6 rounded-2xl border border-white/5">
              <div className="flex justify-between items-end mb-6">
                ...
                <div className="bg-gunmetal px-4 py-1 rounded-md border border-white/10 shadow-inner">
                  <span className="font-mono text-2xl text-sun font-bold">₹{bill.toLocaleString('en-IN')}</span>
                </div>
              </div>
              <input
                id="bill-slider"
            ...
            <div className="mb-8 bg-sky-deep/50 p-6 rounded-2xl border border-white/5">
              <div className="flex justify-between items-end mb-6">
                ...
                <div className="bg-gunmetal px-4 py-1 rounded-md border border-white/10 shadow-inner">
                  <span className="font-mono text-2xl text-teal font-bold">
                    {roof} <span className="text-sm">sq.ft</span>
                  </span>
                </div>
              </div>
              <input
              ...
              {roofTooSmall && (
                <p className="mt-3 text-xs text-amber-400 font-bold">
                  Minimum {MIN_ROOF_FOR_MIN_SYSTEM} sq.ft is recommended for a viable 1 kW system.
                </p>
              )}
            </div>
                ...
                >
                  <div className="p-8 md:p-12 font-mono">
                    <div className="text-center border-b-2 border-dashed border-gray/30 pb-6 mb-8">
                      ...
                      <p className="text-xs text-gray mt-2">SolarEdge Pro // {new Date().toLocaleDateString('en-IN')}</p>
                    </div>
                    ...
                    <div className="border-t-2 border-dashed border-gray/30 pt-8 mb-8">
                      <div className="space-y-4">
                        <ReceiptRow label={t('calculator', 'baseCost')} value={`₹${results.baseCost.toLocaleString('en-IN')}`} />
                        <ReceiptRow label={t('calculator', 'centralSub')} value={`-₹${results.subsidy.toLocaleString('en-IN')}`} />
                        <div className="h-px bg-gray/30 my-2" />
                        <ReceiptRow label={t('calculator', 'finalCost')} value={results.feasible ? `₹${results.finalCost.toLocaleString('en-IN')}` : 'Roof upgrade needed'} isBold />
                        <ReceiptRow label={t('calculator', 'payback')} value={results.feasible ? `${results.payback} Years` : 'Not feasible yet'} isBold />
                        <div className="h-px bg-gray/30 my-2" />
                    ...
                    <div className="mb-8 rounded-xl border border-amber/20 bg-amber/10 px-4 py-3 text-sm text-amber-700 font-medium">
                      {results.message}
                    </div>
                    ...
                    <div className="bg-teal/10 border border-teal/20 rounded-xl p-6 flex items-center justify-between mb-8">
                      <div className="flex items-center gap-4">
                        ...
                        <div>
                          ...
                          <p className="text-xl font-bold">
                            {results.co2} <span className="text-xs text-gray">Tonnes/yr</span>
                          </p>
                        </div>
                      </div>
                      <div className="text-right">
                        ...
                        <p className="text-xl font-bold">
                          {results.trees} <span className="text-xs text-gray">Trees</span>
                        </p>
                      </div>
                    </div>
        ...
        <p className="text-center text-gray/60 text-xs mt-12 max-w-3xl mx-auto leading-relaxed">{t('calculator', 'disclaimer')}</p>
      </div>
    </section>
  );
};

function deriveResidentialUnitsFromBill(bill: number) {
  if (!bill || bill <= 0 || !Number.isFinite(bill)) return 0;
  if (bill <= 536) return bill / 5.36;
  if (bill <= 2448) return 100 + (bill - 536) / 9.56;
  if (bill <= 5192) return 300 + (bill - 2448) / 13.72;
  return 500 + (bill - 5192) / 15.57;
}
